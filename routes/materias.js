...
router.get('/materias',(req,res)=>{
    ...
    mysqlConnection.query('select m.id, m.nombre, m.grado, m.seccion, m.status, m.docentes_id, d.nombre as docente from dte0g8247tlcbkvs.materias m join dte0g8247tlcbkvs.docentes d on d.id = m.docentes_id;',(err,rows,fields)=>{
        if(!err){
            res.send(rows);
        }else{
            console.log(err);
...
router.get('/materias/:id',(req,res)=>{
    ...
    mysqlConnection.query('select m.id, m.nombre, m.grado, m.seccion, m.status, m.docentes_id, d.nombre as docente from dte0g8247tlcbkvs.materias m join dte0g8247tlcbkvs.docentes d on d.id = m.docentes_id where m.id = ?;',[req.params.id],(err,rows,fields)=>{
        if(!err){
            res.send(rows);
        }else{
            console.log(err);
