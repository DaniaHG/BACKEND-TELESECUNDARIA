...
router.get('/periodos',(req,res)=>{
    ...
    mysqlConnection.query('Select id, descripcion from periodos;',(err,rows,fields)=>{
        if(!err){
            res.send(rows);
        }else{
            console.log(err);
...
router.get('/periodos/:id',(req,res)=>{
    ...
    mysqlConnection.query('Select id, descripcion from periodos where id = ?;',[req.params.id],(err,rows,fields)=>{
        if(!err){
            res.send(rows);
        }else{
            console.log(err);
